...
router.get('/candidates', adminAuth, async (req, res, next) => {
  try {
    ...
    if (search) {
      ...
      countQuery += ` AND cp.display_name ILIKE $${countParams.length}`;
    }
...
router.delete('/candidates/:id', adminAuth, async (req, res, next) => {
  try {
    await db.query('DELETE FROM candidate_source_links WHERE candidate_id = $1', [req.params.id]);
    await db.query('DELETE FROM candidacies WHERE candidate_id = $1', [req.params.id]);
    await db.query('DELETE FROM candidate_positions WHERE candidate_id = $1', [req.params.id]);
    await db.query('DELETE FROM follows WHERE candidate_id = $1', [req.params.id]);
    const result = await db.query('DELETE FROM candidate_profiles WHERE id = $1 RETURNING id', [req.params.id]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Candidate not found' });
...
router.post('/ingestion/sync/congress', adminAuth, async (req, res, next) => {
  try {
    ...
    exec(`node scripts/sync-congress-gov.js ${args}${stateArg}`, { cwd: __dirname + '/..', env: process.env }, () => {
...
router.post('/ingestion/sync/bills', adminAuth, async (req, res, next) => {
  try {
    ...
    const { exec } = require('child_process');
    exec(`node scripts/sync-open-states-bills.js ${state}`, { cwd: __dirname + '/..', env: process.env }, () => {
